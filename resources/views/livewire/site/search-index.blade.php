    ...
    <div class="container mt-2 p-l product-category mb-6">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Tìm kiếm: <span class="text-bold"
                        style="font-weight:800">{{ $keyword }}</span></h5>
            </div>
            <div class="card-body">
                @if (!empty($sorts))
                    <div class="demo-inline-spacing filter-product">
                        @foreach ($sorts as $sortKey => $item)
                            <span
                                class="badge bg-label-dark {{ isset($filters['sort']) && $filters['sort'] == $sortKey ? 'active' : '' }}"
                                wire:click='sortProduct("{{ $sortKey }}")'><i
                                    class='bx {{ $item['icon'] }}'></i>{{ $item['title'] }}</span>
                        @endforeach
                    </div>
                @endif
